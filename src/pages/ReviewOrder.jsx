...
const ReviewOrder = () => {
    ...
    ;(async () => {
      try {
        ...
        try {
          ...
          throw new Error('Projects response not JSON')
        }
        try {
          items = await itemsRes.json()
        } catch (err) {
          const text = await itemsRes.text()
          console.error('Items response not JSON:', text)
          throw new Error('Items response not JSON')
        }
        setUserProjects(projects)
        // setPrefilledItems(items)

        // Handle review order data if UUID exists
        if (uuid && reviewOrderRes) {
          if (reviewOrderRes.ok) {
            try {
              ...
              if (Array.isArray(reviewOrderData)) {
                ...
                reviewOrderData &&
                typeof reviewOrderData === 'object' &&
                reviewOrderData !== null
              ) {
                orderItems = [reviewOrderData]
              } else {
                console.warn(
                  'Review order data is not in expected format:',
                  reviewOrderData
                )
                orderItems = []
              }
              setPrefilledItems(orderItems)
            } catch (err) {
              const text = await reviewOrderRes.text()
              console.error('ReviewOrder response not JSON:', text)
              setPrefilledItems([])
              throw new Error('ReviewOrder response not JSON')
            }
          } else {
            console.error('Review order not found')
            setPrefilledItems([])
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const initializeData = () => {
      try {
        // Set customer info from URL parameters
        ...
        setCustomerInfo((prev) => ({ ...prev, ...urlCustomerInfo }))
